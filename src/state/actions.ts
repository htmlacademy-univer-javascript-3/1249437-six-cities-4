...
export const fetchUser = createAsyncThunk<void, undefined, ThunkConfig>(
  'auth/getUser',
  async (_, { extra, dispatch }) => {
...
export const logIn = createAsyncThunk<boolean, { email: string; password: string }, ThunkConfig>(
  'auth/logIn',
...
export const changeFavouriteStatus = createAsyncThunk<void, { offerId: string; newStatus: FavouriteStatus }, ThunkConfig>(
  'favourite/changeStatus',
  async ({ offerId, newStatus }, { extra, dispatch, getState }) => {
    ...
    if (response.status === 200 || response.status === 201) {
      ...
      const newOffers = (offers || []).map((offer) =>
        offer.id === offerId ? { ...offer, isFavorite: response.data.isFavorite } : offer
      );
